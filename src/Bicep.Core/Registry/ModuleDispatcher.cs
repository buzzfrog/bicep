...
namespace Bicep.Core.Registry
{
    public class ModuleDispatcher : IModuleDispatcher
    {
        ...
        {
            if(this.memoryCache.TryGetValue<RestoreFailureInfo>(moduleReference, out var failureInfo))
            {
        ...
        private void SetRestoreFailure(ModuleReference moduleReference, DiagnosticBuilder.ErrorBuilderDelegate failureBuilder)
        {
            ...
            this.memoryCache.Set(moduleReference, new RestoreFailureInfo(failureBuilder), absoluteExpirationRelativeToNow: FailureExpirationInterval);
        }

        private class RestoreFailureInfo
        {
            public RestoreFailureInfo(DiagnosticBuilder.ErrorBuilderDelegate failureBuilder)
            {
